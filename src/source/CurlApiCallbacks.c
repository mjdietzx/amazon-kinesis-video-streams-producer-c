 ...
 */
STATUS createCurlApiCallbacks(PCallbacksProvider pCallbacksProvider, PCHAR region, PCHAR service, API_CALL_CACHE_TYPE cacheType,
                              UINT64 endpointCachingPeriod, PCHAR controlPlaneUrl, PCHAR certPath, PCHAR userAgentNamePostfix, PCHAR customUserAgent,
                              PCurlApiCallbacks* ppCurlApiCallbacks)
{
    ...
    if (controlPlaneUrl == NULL || controlPlaneUrl[0] == '\0') {
        ...
        SNPRINTF(pCurlApiCallbacks->controlPlaneUrl, MAX_URI_CHAR_LEN, "%s%s.%s%s", CONTROL_PLANE_URI_PREFIX, service, pCurlApiCallbacks->region,
                        ...
                        PServiceCallContext pServiceCallContext)
{
    ...
    CHK_STATUS(createCurlRequest(HTTP_REQUEST_VERB_POST, url, paramsJson, streamHandle, pCurlApiCallbacks->region, KINESIS_VIDEO_SERVICE_NAME,
                     ...
                     BOOL acksEnabled, PCHAR streamingEndpoint, PServiceCallContext pServiceCallContext)
{
    ...
    CHK_STATUS(createCurlRequest(HTTP_REQUEST_VERB_POST, url, NULL, (STREAM_HANDLE) pServiceCallContext->customData, pCurlApiCallbacks->region,
                                 KINESIS_VIDEO_SERVICE_NAME, currentTime, CURL_API_DEFAULT_CONNECTION_TIMEOUT, pServiceCallContext->timeout,
                                 pServiceCallContext->callAfter, pCurlApiCallbacks->certPath, pCredentials, pCurlApiCallbacks, &pCurlRequest));
